...
int main() {
    check(stableServerLevelID(123,456,false)==123,"Positive level ID must win over original ID");
    check(stableServerLevelID(0,456,false)==456,"Non-editor transient zero ID should recover original server ID");
    check(stableServerLevelID(0,456,true)==0,"Editor copy must not inherit original server macro library");
    check(stableServerLevelID(-1,0,false)==0,"Invalid placeholder IDs must stay local");
