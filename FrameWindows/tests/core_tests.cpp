...
int main() {
    ...
    require(canShift(r, 0, 240), "same-tick press/release must be simulated");
    ...
    require(trialEndpoint(next,0,true)==165,"Cube endpoint skips replay-only release and settles after the next active player input");
    require(trialEndpoint(next,1,true)==165,"Replay-only release would point to the next active input group");
    require(trialEndpoint(next,3,true)==441,"Later input has its own next-click endpoint");
    ...
    require(trialEndpoint(next,3,true)==405,"Wave press executes release and settling steps");
    require(trialEndpoint(next,4,true)==441,"Wave release stops at next press");
    require(trialEndpoint(next,0,true)==165,"Non-wave input shares the next active input group endpoint");
    ...
    require(trialEndpoint(filtered,2,true)==235,"ship press endpoint is its next control transition");
    require(trialEndpoint(filtered,3,true)==305,"ship release endpoint is the next control transition");
    ...
    require(repeatableFailure(false,120,120),"Identical boundary replay accepted");
    ...
    require(search.results[2].overflow && search.results[2].width() == 41, "both wide sides are independently censored at 20 offsets");
