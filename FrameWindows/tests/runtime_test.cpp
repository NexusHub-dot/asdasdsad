...
// Enable FWL_RUNTIME_TEST and launch GD with --geode:alan.pulse_macro.self-test.
...
class $modify(FWLTestBase,GJBaseGameLayer) {
    void processQueuedButtons(float dt,bool clear) {
        auto& s=fwl::session();int tick=s.physicsTick;
    ...
    void update(float dt) {
        ...
        if(s.mode==fwl::Mode::Recording && s.physicsTick>=360 && !requested) {
        ...
        if(analyzing && s.mode==fwl::Mode::Idle) {
            ...
            if(ok)for(auto const& w:s.reference->windows)ok=ok && (w.ignored || (w.measured && w.overflow));
            report(ok?"PASS: real-engine reference replay, four replay events, relevant 21+ windows, final baseline, and persistence.":"FAIL: "+s.reason);
        }
    }
};
