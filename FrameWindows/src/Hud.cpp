...
void Hud::render() {
    ...
    std::string modeText=run && run->nextClick?"NEXT INPUT + SETTLING":"FULL MACRO";
    ...
    if(run && (run->uncertain || run->finalValidationWarning)) {
        modeText="UNVERIFIED - F4";
        footer->setVisible(true); // Warnings survive reload, including minimal HUD.
    }
    ...
    for(auto it=start;it!=run->inputs.end() && it->tick<=tick && drawn<100;++it) {
        ...
        text->setString((w.measured?(std::to_string(w.width())+((w.earlyCensored||w.lateCensored)?"+":"")):std::string("?")).c_str());
        ++drawn;
    }
}
}
