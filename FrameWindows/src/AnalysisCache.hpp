...
std::optional<size_t> selectAnalysisCheckpoint(Points const& points, int targetTick,
    ...
    int64_t latest = static_cast<int64_t>(targetTick) - std::max(0, window) - std::max(1, safety);
...
inline int candidateRestoreBoundary(Run const& run,size_t index,int offset,
    ...
    int64_t shifted=static_cast<int64_t>(original)+offset;
    auto boundary=std::min<int64_t>(original,shifted)-std::max(1,safety);
    return static_cast<int>(std::clamp<int64_t>(boundary,-1,std::numeric_limits<int>::max()));
}
template<class Points>
