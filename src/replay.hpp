...
inline int stableServerLevelID(int levelID, int originalLevelID, bool editorLevel) {
    // Online/saved level objects occasionally surface with m_levelID == 0
    // while entering PlayLayer. m_originalLevel still carries the server ID in
    // several of those variants. Never use it for editor copies: an edited
    // copy must keep its own local macro library.
    if (levelID > 0) return levelID;
    if (!editorLevel && originalLevelID > 0) return originalLevelID;
    return 0;
}
