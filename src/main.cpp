...
std::string normalizedLevelName(std::string value) {
    std::string out;
    out.reserve(value.size());
    bool space=false;
    for(unsigned char c:value) {
        if(std::isalnum(c)) { out.push_back(static_cast<char>(std::tolower(c))); space=false; }
        else if(!out.empty() && !space) { out.push_back(' '); space=true; }
    }
    while(!out.empty() && out.back()==' ')out.pop_back();
    return out;
}
std::string fingerprintAliasKey(std::uint64_t fp) { return fmt::format("library-alias-fp-{:016x}",fp); }
std::string descriptorAliasKey(std::string const& name,std::string const& creator) {
    return fmt::format("library-alias-desc-{:016x}",pulse::fingerprint(normalizedLevelName(name)+"\n"+normalizedLevelName(creator)));
}
int aliasValue(std::string const& key) {
    auto value=Mod::get()->getSavedValue<int64_t>(key,0);
    return value>0 && value<=INT_MAX?static_cast<int>(value):0;
}
bool rememberAlias(std::string const& key,int levelID) {
    if(levelID<=0)return false;
    auto old=Mod::get()->getSavedValue<int64_t>(key,0);
    // -1 means ambiguous. Never let a weak alias silently bounce between two
    // different server levels that happen to share a descriptor/content.
    if(old==0) { Mod::get()->setSavedValue<int64_t>(key,levelID); return true; }
    if(old>0 && old!=levelID) { Mod::get()->setSavedValue<int64_t>(key,-1); return true; }
    return false;
}
std::vector<std::filesystem::path> playableMacroFilesForSlot(int slot) {
    std::vector<std::filesystem::path> found;
    if(!std::filesystem::exists(macroDir()))return found;
    auto suffix=fmt::format("-slot{}.pulse",slot);
    for(auto const& entry:std::filesystem::directory_iterator(macroDir())) {
        if(!entry.is_regular_file())continue;
        auto name=entry.path().filename().string();
        if(name.ends_with(suffix))found.push_back(entry.path());
    }
    return found;
}
int discoverStableIDFromLibrary(std::uint64_t fp,std::string const& levelName) {
    if(!std::filesystem::exists(macroDir()))return 0;
    std::set<int> exactIDs,nameIDs;
    auto normalized=normalizedLevelName(levelName);
    for(auto const& entry:std::filesystem::directory_iterator(macroDir())) {
        if(!entry.is_regular_file() || entry.path().extension()!=".pulse")continue;
        try {
            auto candidate=pulse::load(entry.path());
            if(candidate.levelID<=0)continue;
            if(candidate.fingerprint==fp)exactIDs.insert(candidate.levelID);
            if(!normalized.empty() && normalizedLevelName(candidate.levelName)==normalized)nameIDs.insert(candidate.levelID);
        } catch(...) {}
    }
    if(exactIDs.size()==1)return *exactIDs.begin();
    // Name-only recovery is intentionally only accepted when unique across the
    // user's Pulse library. This recovers a transient zero-ID/load variant
    // without merging two different same-named server levels.
    if(exactIDs.empty() && nameIDs.size()==1)return *nameIDs.begin();
    return 0;
}
int canonicalLevelID(GJGameLevel* level,std::uint64_t fp) {
    if(!level)return 0;
    const int rawID=level->m_levelID.value();
    const int originalID=level->m_originalLevel.value();
    const bool editorLevel=level->m_levelType==GJLevelType::Editor;
    int id=pulse::stableServerLevelID(rawID,originalID,editorLevel);
    auto name=std::string(level->m_levelName);
    auto creator=std::string(level->m_creatorName);
    if(id<=0 && !editorLevel) {
        id=aliasValue(fingerprintAliasKey(fp));
        if(id<=0)id=aliasValue(descriptorAliasKey(name,creator));
        if(id<=0)id=discoverStableIDFromLibrary(fp,name);
    }
    if(id>0 && !editorLevel) {
        bool changed=rememberAlias(fingerprintAliasKey(fp),id);
        if(!name.empty() || !creator.empty())changed=rememberAlias(descriptorAliasKey(name,creator),id) || changed;
        if(changed) {
            auto saved=Mod::get()->saveData();
            if(!saved)log::warn("Could not persist Pulse level-library aliases");
        }
    }
    return id;
}
bool strongLibraryMatch(pulse::Tape const& candidate,pulse::Tape const& current) {
    if(current.levelID>0) {
        if(candidate.levelID==current.levelID)return true;
        // Recover a take recorded during a transient zero-ID state when the
        // actual level data still matches exactly.
        return candidate.fingerprint==current.fingerprint;
    }
    // Local/editor libraries stay separated by both content and name so two
    // unrelated editor levels with identical object data do not collapse.
    return candidate.levelID<=0 && candidate.fingerprint==current.fingerprint &&
        normalizedLevelName(candidate.levelName)==normalizedLevelName(current.levelName);
}
...
std::filesystem::path resolveMacroPath(pulse::Tape const& tape,int slot,bool migrate=true) {
    ...
    if(std::filesystem::is_regular_file(stable))matches.push_back(stable);
    auto legacy=legacyMacroPath(tape,slot);
    if(std::filesystem::is_regular_file(legacy) && legacy!=stable)matches.push_back(legacy);
    for(auto const& path:playableMacroFilesForSlot(slot)) {
        if(std::find(matches.begin(),matches.end(),path)!=matches.end())continue;
        try {
            auto candidate=pulse::load(path);
            if(strongLibraryMatch(candidate,tape))matches.push_back(path);
        } catch(...) {}
    }
    if(matches.empty())return stable;
    std::sort(matches.begin(),matches.end(),[](auto const& a,auto const& b){
        return std::filesystem::last_write_time(a)>std::filesystem::last_write_time(b);
    });
    auto selected=matches.front();
    if(!migrate)return selected;

    auto chosen=pulse::load(selected);
    const bool needsCanonicalHeader=tape.levelID>0 && chosen.levelID!=tape.levelID;
    const bool selectedAlreadyStable=selected==stable;
    if(needsCanonicalHeader || !selectedAlreadyStable) {
        // If a recording was made while GD exposed the same online level as a
        // zero-ID/local-looking PlayLayer, promote that take into the stable
        // server-ID library. saveRecordingFile is atomic and preserves the old
        // stable take as .bak when one exists.
        if(tape.levelID>0)chosen.levelID=tape.levelID;
        std::filesystem::create_directories(stable.parent_path());
        pulse::saveRecordingFile(stable,chosen);
        if(selected!=stable) {
            try{retireLegacyMacro(selected);}catch(std::exception const& e){log::warn("Could not retire split-library macro {}: {}",selected.string(),e.what());}
        }
        log::info("Pulse merged slot {} into stable library {}",slot,pulse::stableLevelToken(tape));
    }
    for(auto const& duplicate:matches) {
        if(duplicate==stable || duplicate==selected)continue;
        try{retireLegacyMacro(duplicate);}catch(std::exception const& e){log::warn("Could not retire duplicate macro {}: {}",duplicate.string(),e.what());}
    }
    return stable;
}
...
pulse::Tape identity(PlayLayer* layer) {
    ...
    tape.levelName = std::string(level->m_levelName);
    tape.fingerprint = pulse::fingerprint(std::string(level->m_levelString));
    tape.levelID = canonicalLevelID(level,tape.fingerprint);
    tape.seed = layer->m_randomSeed;
    tape.fixedUpdates = session.fixedUpdates;
    log::debug("Pulse library identity: raw={} original={} type={} -> stable={} hash={:016x}",
        level->m_levelID.value(),level->m_originalLevel.value(),static_cast<int>(level->m_levelType),tape.levelID,tape.fingerprint);
    return tape;
}
...
void apply(pulse::Input input) {
    ...
    if(session.mode==Mode::Record && session.frameLinked && !session.resetting) {
        ...
        msg.sourceEventIndex=static_cast<int>(session.recorder.tape.events.size())-1;
...
void saveRecording() {
    ...
    // Re-resolve at save time too. Some GD/mod entry paths populate the stable
    // server identity only after PlayLayer has already started; a recording
    // begun during that transient state must still land in the normal library.
    if(session.layer && PlayLayer::get()==session.layer) {
        auto current=identity(session.layer);
        if(current.levelID>0)session.recorder.tape.levelID=current.levelID;
    }
    auto path=resolveMacroPath(session.recorder.tape,session.slot);
    pulse::saveRecordingFile(path,session.recorder.tape);
    session.dirty=false;
    log::info("Saved macro slot {} to {}: {} steps, {} inputs",session.slot,path.filename().string(),session.recorder.tape.steps.size(),session.recorder.tape.events.size());
}
void endFrameLink(bool completed, bool analyze, bool saved = false) {
    ...
    if(msg.savedMacro) {
        try{msg.macroRevision=pulse::revision(resolveMacroPath(session.recorder.tape,session.slot));}
...
void commitRepair() {
    try {
        ...
        auto current=identity(session.layer);
        if(current.levelID>0)repaired.levelID=current.levelID;
        auto path=resolveMacroPath(current,session.slot);
